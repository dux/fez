...
describe("Fez.index reserved names", () => {
  test("ensure does not clobber a helper method", () => {
    const Fez = getFez();
    Fez.index.ensure("info").source = "reserved-source";

    expect(typeof Fez.index.info).toBe("function");
    expect(Fez.index.get("info").source).toBe("reserved-source");
    expect(Fez.index.names()).not.toContain("_entry_info");

    delete Fez.index._entry_info;
  });
});
