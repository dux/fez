...
describe("GlobalState", () => {
  ...
  test("a __proto__ key does not touch Object.prototype", () => {
    GlobalState.set("__proto__", { polluted: true });
    expect({}.polluted).toBeUndefined();
    expect(GlobalState.get("__proto__")).toEqual({ polluted: true });
  });

  test("subscribe with a non-function is a no-op, not a throw", () => {
    const key = uniq("bad-sub");
    const origError = console.error;
    console.error = () => {};
    let unsub;
    try {
      unsub = GlobalState.subscribe(key, "not-a-function");
      GlobalState.set(key, 1);
    } finally {
      console.error = origError;
    }
