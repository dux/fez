...
describe('Old to new syntax conversion', () => {
  ...
  describe('conditionals', () => {
    ...
    test('converts {{elsif}} to {:else if}', () => {
      const html = render('{{if state.x === 1}}one{{elsif state.x === 2}}two{{else}}other{{/if}}', {
        state: { x: 2 },
      });
      expect(html).toBe('two');
    });
  ...
  describe('blocks', () => {
    test('converts {{block name}}...{{/block}} and {{block:name}}', () => {
      const html = render('{{block avatar}}<img/>{{/block}}<div>{{block:avatar}}</div>', {
        state: {},
      });
    ...
    test('block with expressions inside', () => {
      const html = render('{{block item}}<li>{{state.name}}</li>{{/block}}{{block:item}}', {
        state: { name: 'Test' },
      });
