...
describe("Old to new syntax conversion", () => {
  describe("expressions", () => {
    ...
    test("leaves [[ ]] literal (array/JSON content is not rewritten)", () => {
      const html = render("[[1, 2, 3]]", { state: {} });
  ...
  describe("malformed loops", () => {
    test("{#each} without ' as ' throws in strict mode", () => {
      expect(() => createTemplate("{#each state.items}<b>{i}</b>{/each}", { strict: true })).toThrow(
        /missing " as "/,
      );
    });

    test("{#for} without ' in ' throws in strict mode", () => {
      expect(() => createTemplate("{#for item state.items}<b>{item}</b>{/for}", { strict: true })).toThrow(
        /missing " in "/,
      );
    });
  });
