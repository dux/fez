...
test("localStorage - setting undefined removes the key", () => {
  Fez.localStorage.set('to_remove', 'x');
  Fez.localStorage.set('to_remove', undefined);

  expect(Fez.localStorage.get('to_remove', 'fallback')).toBe('fallback');
});

test("localStorage - remove and clear do not throw", () => {
  Fez.localStorage.set('a', 1);
  expect(() => Fez.localStorage.remove('a')).not.toThrow();
  expect(() => Fez.localStorage.clear()).not.toThrow();
});
