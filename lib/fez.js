...
class FezBase {
  ...
  renderString(text, connext) {
    if (typeof text == 'object') {
      text = text[0]
    }

    text = text.replaceAll('$$.', this.fezHtmlRoot)
    text = Mustache.render(text, connext || this)
  ...
  html(text) {
    ...
    newNode.innerHTML = this.renderString(text)
