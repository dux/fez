...
const VERBATIM =
  /^@(-\w+-)?(keyframes|font-face|property|counter-style|page|namespace|font-feature-values|viewport)\b/i;
...
function parse(css) {
  ...
  for (let i = 0; i < css.length; i++) {
    ...
    if (ch === '}') {
      const tail = buf.trim();
      if (tail) {
        stack[stack.length - 1].declarations.push(tail);
      }
      buf = '';
      if (stack.length > 1) {
        stack.pop();
...
const wrap = (conditions, inner) => conditions.reduceRight((acc, cond) => `${cond}{${acc}}`, inner);
