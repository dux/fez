...
type: ${l}`),t.push(l),!!document.getElementById("dump-dialog")?(n=e.length-1,localStorage.setItem("_LOG_INDEX",n),i&&i()):r()}})();typeof window<"u"&&!window.LOG&&(window.LOG=Y);var Ae=Y;var _e=()=>{let e=parseInt(window.location.port)||80;if(!(Fez.DEV===!0||e>2999&&Fez.DEV!==!1))return;let t=document.querySelectorAll(".fez-highlight-overlay");if(t.length>0){t.forEach(i=>i.remove());return}document.querySelectorAll(".fez, .svelte").forEach(i=>{let s=null,r=null;if(i.classList.contains("fez")&&i.fez&&i.fez.fezName?(s=i.fez.fezName,r="fez"):i.classList.contains("svelte")&&i.svelte&&i.svelte.svelteName&&(s=i.svelte.svelteName,r="svelte"),s){let o=document.createElement("div");o.className="fez-highlight-overlay";let l=i.getBoundingClientRect(),a=window.pageYOffset||document.documentElement.scrollTop,d=window.pageXOffset||document.documentElement.scrollLeft;o.style.cssText=`
      ...
      `,f.addEventListener("click",h=>{h.stopPropagation(),Fez.log(i)}),o.appendChild(f),document.body.appendChild(o)}})};document.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="e"&&(e.target.closest("form")||(e.preventDefault(),_e()))});var Ie=_e;var yt=new Set(["area","base","br","col","embed","hr","img","input","link","meta","source","track","wbr"]);function L(e){return e.replace(/<([a-z][a-z-]*)\b((?:=>|[^>])*)>/g,(t,n,i)=>!i.trimEnd().endsWith("/")||yt.has(n)?t:`<${n}${i.replace(/\s*\/$/,"")}></${n}>`)}var zt=new MutationObserver(e=>{for(let t of e)if(t.type==="attributes"){let n=t.target.fez;if(n){let i=t.attributeName,s=t.target.getAttribute(i);n.props[i]=s,n.onPropsChange(i,s)}}});function Q(e,t){let n=globalThis.window?.Fez||globalThis.Fez;if(!e.includes("-")){console.error(`Fez: Invalid name "${e}". Must contain a dash.`);return}t=wt(n,e,t),t.html&&(/<slot\s[^>]*unwrap[\s>\/]/.test(t.html)&&(t.fezSlotUnwrap=!0),t.html=t.html.replace(/<slot(\s[^>]*)?>/,'<div class="fez-slot" fez-keep="default-slot"$1>').replace("</slot>","</div>"),t.fezHtmlFunc=I(t.html,{name:e})),t.css&&n.globalCss(t.css,{name:e}),t.cssGlobal&&n.globalCss(t.cssGlobal),n.index.ensure(e).class=t,customElements.get(e)||customElements.define(e,class extends HTMLElement{connectedCallback(){document.readyState==="loading"?requestAnimationFrame(()=>Le(e,this)):Le(e,this)}})}function wt(e,t,n){if(n.prototype instanceof T)return n.html&&(n.html=L(n.html)),n;let i=new n,s=class extends T{},r=[...Object.getOwnPropertyNames(i),...Object.getOwnPropertyNames(n.prototype)].filter(l=>l!=="constructor"&&l!=="prototype");for(let l of r)s.prototype[l]=i[l];let o={GLOBAL:"GLOBAL",NAME:"nodeName"};for(let[l,a]of Object.entries(o))i[l]&&(s[a]=i[l]);if(i.CSS&&(s.css=typeof i.CSS=="function"?i.CSS():i.CSS),i.CSS_GLOBAL&&(s.cssGlobal=typeof i.CSS_GLOBAL=="function"?i.CSS_GLOBAL():i.CSS_GLOBAL),i.HTML){let l=typeof i.HTML=="function"?i.HTML():i.HTML;s.html=L(l)}return i.META&&(s.META=i.META,e.index.ensure(t).meta=i.META),i.GLOBAL&&e.onReady(()=>document.body.appendChild(document.createElement(t))),e.consoleLog(`${t} compiled`),s}function Le(e,t){if(!t.isConnected||t.classList?.contains("fez"))return;let n=Fez.index[e]?.class,i=typeof n.nodeName=="function"?n.nodeName(t):n.nodeName,s=document.createElement(i||"div");if(s.classList.add("fez",`fez-${e}`),!t.parentNode){console.warn(`Fez: ${e} has no parent, skipping`);return}t.parentNode.replaceChild(s,t);let r=new n;r.UID=++Fez.instanceCount,Fez.instances.set(r.UID,r),r.oldRoot=t,r.fezName=e,r.root=s,r.props=n.getProps(t,s),r.class=n,s._fezSignature=t.outerHTML,r.fezSlot(t,s),s.fez=r,n.GLOBAL&&n.GLOBAL!==!0&&(window[n.GLOBAL]||=r),window.$&&(r.$root=$(s)),r.props.id&&s.setAttribute("id",r.props.id);let o=t.getAttribute("key");o&&s.setAttribute("key",o),t._fezKey!==void 0&&(s._fezKey=t._fezKey);let l=t.getAttribute("fez-key");l&&s.setAttribute("fez-key",l);let a=t.getAttribute("fez-keep");if(a&&s.setAttribute("fez-keep",a),r.fezRegister(),r.root.childNodes.length&&(r._fezSlotNodes=Array.from(r.root.childNodes),r._fezChildNodes=r._fezSlotNodes.filter(f=>f.nodeType===1)),r._isInitializing=!0,(r.onInit||r.init||r.created||r.connect).call(r,r.props),r.fezRender(),r._isInitializing=!1,r.onMount(r.props),r.onRefresh(r.props),r.onSubmit){let f=r.root.nodeName==="FORM"?r.root:r.find("form");f&&(f.onsubmit=h=>{h.preventDefault(),r.onSubmit(r.formData())})}if(r.onPropsChange){zt.observe(s,{attributes:!0});for(let[f,h]of Object.entries(r.props))r.onPropsChange(f,h)}}var xt=/^@(-\w+-)?(keyframes|font-face|property|counter-style|page|import|namespace|font-feature-values)\b/i,ke=e=>e.replace(/:global\(([^)]*)\)/g,"$1").replace(/\s+/g," ").trim(),vt=e=>{let t=e.indexOf("{");return t<0?ke(e):`${ke(e.slice(0,t))} ${e.slice(t)}`},ee=e=>{let t=0;for(let n of e)n==="{"?t++:n==="}"&&t--;return t};function te(e){if(!e)return{scoped:"",global:""};if(!e.includes(":global(")&&!/(^|\n)\s*@/.test(e))return{scoped:e,global:""};let t=e.split(`
`),n=[],i=[],s=0,r=null;for(let o of t){let l=o.replace(/\/\*.*?\*\//g,""),a=l.trim();if(r){s+=ee(l),r.body.push(o),s===0&&(i.push(r.body.join(`
`)),r=null);continue}if(s===0&&a){let d=a.includes(":global("),f=xt.test(a);if(d||f){if(f&&!a.includes("{")){i.push(a);continue}let h=d?vt(a):a,b=ee(l);if(b===0){i.push(h);continue}if(b>0){r={body:[h]},s=b;continue}}}s+=ee(l),n.push(o)}return r?{scoped:e,global:""}:{scoped:n.join(`
`),global:i.join(`
`)}}var Oe=new Map;function Me(e){let t=e.split(`
`),n=t.filter(s=>s.trim());if(!n.length)return e;let i=Math.min(...n.map(s=>s.match(/^(\s*)/)[1].length));return i===0?e:t.map(s=>s.slice(i)).join(`
`)}function Re(e,t){return new RegExp(`^<${t}(?:\\s|>|$)`,"i").test(e)}var Et=/(?:^|\s)global(?:\s*=\s*(?:""|''|"global"|'global'|global))?(?=\s|$)/i;function Ct(e){let t=e.match(/^<style\b([^>]*?)\/?>/i);return!!t&&Et.test(t[1])}var N={body:"body { } in a scoped <style>. Move these rules to <style global>.",host:":host is not supported. <style> is already scoped - use `&` for the root node.",fez:":fez is no longer an author-facing selector. <style> is already scoped - use `&` for the root node.",globalInGlobal:":global() inside <style global>. These rules are already global - drop the wrapper."};function $t(e){return e.replace(/\/\*[\s\S]*?\*\//g,t=>t.replace(/[^\n]/g," ")).replace(/^([ \t]*)\/\/[^\n]*/gm,(t,n)=>n+" ".repeat(t.length-n.length))}function De(e,t,n){if(!t)return;let i=$t(t),s=r=>{throw new Error(`<${e}> style error: ${r}`)};!n&&/(?:^|\s)body\s*\{/.test(i)&&s(N.body),/:host\b/.test(i)&&s(N.host),/:fez\b/.test(i)&&s(N.fez),n&&/:global\(/.test(i)&&s(N.globalInGlobal)}function Tt(e){if(!e)return!1;let t=e.replace(/<demo>[\s\S]*?<\/demo>/gi,"");return/<(xmp|template)\s+fez\s*=/i.test(t)}function k(e,t){if(arguments.length===1)return se(e);if(Tt(t)){if(e){Fez.index.ensure(e).source=t;let s=ie(t);s.info?.trim()&&(Fez.index.ensure(e).info=s.info),s.demo?.trim()&&(Fez.index.ensure(e).demo=s.demo)}return se(t)}if(e&&!e.includes("-")&&!e.includes(".")&&!e.includes("/")){console.error(`Fez: Invalid name "${e}". Must contain a dash (e.g., 'my-element').`);return}if(Fez.index.ensure(e).source=t,Oe.get(e)?.html===t&&Fez.index[e]?.class)return Fez.index[e].class;let i=_t(e,ie(t));return kt(e),It(e,i),Oe.set(e,{html:t}),Fez.index[e]?.class}function se(e){if(e instanceof Node){let n=e;n.remove();let i=n.getAttribute("fez");if(i?.includes(".")||i?.includes("/"))return St(i);if(i&&!i.includes("-")){console.error(`Fez: Invalid name "${i}". Must contain a dash.`);return}return k(i,n.innerHTML)}(e?Fez.domRoot(e):document.body).querySelectorAll("template[fez], xmp[fez]").forEach(n=>se(n))}function St(e){if(Fez.consoleLog(`Loading from ${e}`),e.endsWith(".txt")){Fez.head({fez:e});return}Fez.fetch(e).then(t=>{let i=new DOMParser().parseFromString(t,"text/html").querySelectorAll("template[fez], xmp[fez]");if(i.length>0){let s=e.split("/").pop().split(".")[0],r=ie(t);r.info?.trim()&&(Fez.index.ensure(s).info=r.info),r.demo?.trim()&&(Fez.index.ensure(s).demo=r.demo),i.forEach(o=>{let l=o.getAttribute("fez");if(l&&!l.includes("-")&&!l.includes(".")&&!l.includes("/")){console.error(`Fez: Invalid name "${l}". Must contain a dash.`);return}k(l,o.innerHTML)})}else{let s=e.split("/").pop().split(".")[0];k(s,t)}}).catch(t=>{Fez.onError("compile",`Load error for "${e}": ${t.message}`)})}function ie(e){let t={script:"",style:"",styleGlobal:"",html:"",head:"",demo:"",info:""},n=e.split(`
`),i=[],s="";for(let r of n){let o=r.trim();o.startsWith("<demo")&&!t.demo&&!s?s="demo":o.startsWith("<info")&&!t.info&&!s?s="info":o.startsWith("<script")&&!t.script&&s!=="head"&&s!=="demo"&&s!=="info"?s="script":Re(o,"head")&&!t.head&&s!=="demo"&&s!=="info"?s="head":Re(o,"style")&&s!=="demo"&&s!=="info"?s=Ct(o)?"styleGlobal":"style":o.endsWith("</demo>")&&s==="demo"?(t.demo=Me(i.join(`
