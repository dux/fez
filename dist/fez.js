...
`);t.wrap&&(n=`:fez { ${n} }`),n=n.replace(/\:fez|\:host/,`.fez.fez-${t.name}`),e=b.cssClass(n)}return b.onReady(()=>document.body.parentElement.classList.add(e)),e};J(b);b.subscribe=he;b.publish=pe;b.localStorage=Me;b.fezAwait=se;b.consoleError=(e,t)=>{if(e=`Fez: ${e}`,console.error(e),t)return`<span style="border: 1px solid red; font-size: 14px; padding: 3px 7px; background: #fee; border-radius: 4px;">${e}</span>`};b.consoleLog=e=>{b.LOG&&console.log(`Fez: ${String(e).substring(0,180)}`)};b.onError=(e,t,n)=>{let i=n?.componentName||n?.name;if(!i&&typeof t=="string"){let a=t.match(/<([^>]+)>/);a&&(i=a[1])}let s=i?` [${i}]`:"",r=typeof t=="string"?t:t?.message||String(t),o=`Fez ${e}:${s} ${r}`;return n&&b.LOG?console.error(o,n):console.error(o),t instanceof Error&&t.stack&&b.LOG&&console.error(t.stack),o};De(b);Pe(b);b.compile=O;b.createTemplate=k;b.state=Oe;b.log=Se;b.highlightAll=_e;b.onReady(()=>b.consoleLog("Fez.LOG === true, logging enabled."));var M=b;typeof window<"u"&&(window.FezBase=T,window.Fez=M);Promise.resolve().then(()=>Be());var Dt=new MutationObserver(e=>{for(let{addedNodes:t,removedNodes:n}of e)t.forEach(i=>{i.nodeType===1&&(i.matches?.("template[fez], xmp[fez], script[fez]")&&(M.compile(i),i.remove()),i.querySelectorAll?.("template[fez], xmp[fez], script[fez]").forEach(s=>{M.compile(s),s.remove()}))}),n.forEach(i=>{if(i.nodeType!==1)return;let s=r=>{r.fez&&!r.fez._destroyed&&queueMicrotask(()=>{!r.isConnected&&r.fez&&!r.fez._destroyed&&(M.instances.delete(r.fez.UID),r.fez.fezOnDestroy())})};s(i),i.querySelectorAll?.(".fez")?.forEach(s)})});Dt.observe(document.documentElement,{childList:!0,subtree:!0});var Hn=M;})();
